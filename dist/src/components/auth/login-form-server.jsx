...
function SubmitButton() {
    ...
    return (<button_1.Button type="submit" className="w-full" disabled={pending}>
      {pending ? (<>
...
function LoginFormServer() {
    var _a, _b;
    const [state, formAction] = (0, react_dom_1.useFormState)(auth_1.loginAction, {
        errors: {},
        message: "",
    });
    return (<card_1.Card className="w-full max-w-md mx-auto">
      ...
      <card_1.CardContent>
        <form action={formAction} className="space-y-4">
          {state.message && (<alert_1.Alert variant="destructive">
          ...
          <div className="space-y-2">
            ...
            <input_1.Input id="username" name="username" placeholder="กรอกชื่อผู้ใช้" required/>
            {((_a = state.errors) === null || _a === void 0 ? void 0 : _a.username) && (<p className="text-sm text-red-500">{state.errors.username}</p>)}
          </div>

          <div className="space-y-2">
            <label_1.Label htmlFor="pwd">รหัสผ่าน</label_1.Label>
            <input_1.Input id="pwd" name="pwd" type="password" placeholder="กรอกรหัสผ่าน" required/>
            {((_b = state.errors) === null || _b === void 0 ? void 0 : _b.pwd) && (<p className="text-sm text-red-500">{state.errors.pwd}</p>)}
          </div>
